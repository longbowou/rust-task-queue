...
impl Task for FastTask {
    ...
    fn name(&self) -> &str { "fast_task" }
    fn timeout_seconds(&self) -> u64 { 1 }
}
...
impl Task for CpuIntensiveTask {
    ...
    fn name(&self) -> &str { "cpu_intensive_task" }
...
impl Task for MemoryIntensiveTask {
    ...
    fn name(&self) -> &str { "memory_intensive_task" }
...
fn bench_task_execution_types(c: &mut Criterion) {
    ...
    group.bench_function("cpu_intensive_task_creation", |b| {
        b.iter(|| {
            let task = CpuIntensiveTask { id: 1, iterations: 100 };
            black_box(task);
        })
    });
...
fn bench_task_serialization_performance(c: &mut Criterion) {
    ...
    let cpu_task = CpuIntensiveTask { id: 1, iterations: 1000 };
...
fn bench_concurrent_task_creation(c: &mut Criterion) {
    ...
    for batch_size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &batch_size| {
                b.iter(|| {
                    let tasks: Vec<FastTask> = (0..batch_size)
                        .map(|i| FastTask { id: i as u32 })
                        .collect();
                    black_box(tasks);
                })
            }
            ...
            |b, &batch_size| {
                b.iter(|| {
                    let tasks: Vec<CpuIntensiveTask> = (0..batch_size)
                        .map(|i| CpuIntensiveTask { id: i as u32, iterations: 100 })
                        .collect();
                    black_box(tasks);
                })
            }
