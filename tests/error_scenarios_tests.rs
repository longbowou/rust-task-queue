...
async fn test_redis_connection_failure_recovery() {
    ...
    if let Err(e) = result {
        assert!(
            e.to_string().contains("Connection") || e.to_string().contains("Failed"),
...
async fn test_malformed_task_handling() {
    ...
    registry
        .register_with_name::<TimeoutTask>("timeout_task")
        .expect("Failed to register task");

    task_queue
        .start_workers_with_registry(1, registry.clone())
        .await
    ...
    let _task_id = task_queue
        ...
        .await
        .expect("Failed to enqueue valid task");
    ...
    assert!(
        processed,
        "Valid task should be processed after malformed data"
    );
...
async fn test_task_timeout_handling() {
    ...
    println!(
        "Metrics after timeout: processed={}, failed={}, pending={}",
...
async fn test_worker_panic_recovery() {
    ...
    registry
        .register_with_name::<PanicTask>("panic_task")
        .expect("Failed to register task");

    task_queue
        .start_workers_with_registry(3, registry)
        .await
        .expect("Failed to start workers"); // Use 3 workers for redundancy

    // First, enqueue a normal task to verify basic functionality
    let normal_task_first = PanicTask {
        should_panic: false,
    };
    let _normal_task_id = task_queue
        .enqueue(normal_task_first, queue_names::DEFAULT)
        .await
        .expect("Failed to enqueue normal task");
    ...
    for _i in 0..3 {
        let normal_task = PanicTask {
            should_panic: false,
        };
        let _normal_task_id = task_queue
            .enqueue(normal_task, queue_names::DEFAULT)
            .await
            .expect("Failed to enqueue normal task");
    }
    ...
    let _panic_task_id = task_queue
        .enqueue(panic_task, queue_names::DEFAULT)
        .await
        .expect("Failed to enqueue panic task");
    ...
    for _i in 0..3 {
        ...
        let _normal_task_id = task_queue
            .enqueue(normal_task, queue_names::DEFAULT)
            .await
            .expect("Failed to enqueue normal task after panic");
    }
    ...
    let final_metrics = task_queue
        ...
        .expect("Failed to get metrics");
    println!(
        "Panic recovery test final metrics: processed={}, failed={}",
    ...
    assert!(
        final_metrics.processed_tasks >= 3,
    ...
    let recovery_task = PanicTask {
        should_panic: false,
    };
    let recovery_result = task_queue
        .enqueue(recovery_task, queue_names::DEFAULT)
        .await;
    assert!(
        recovery_result.is_ok(),
        "System should still accept tasks after panic handling"
    );
...
async fn test_concurrent_worker_scaling_edge_cases() {
    ...
    // Start workers
    task_queue
        .start_workers_with_registry(2, registry.clone())
        .await
        .expect("Failed to start workers");
...
async fn test_memory_usage_and_leaks() {
    ...
    registry
        .register_with_name::<ResourceLeakTask>("resource_leak_task")
        .expect("Failed to register task");
    ...
    for _i in 0..5 {
        ...
        task_queue
            .enqueue(task, queue_names::DEFAULT)
            .await
            .expect("Failed to enqueue resource task");
    }
    ...
    while attempts < 100 {
        let queue_metrics = task_queue
            .broker
            .get_queue_metrics(queue_names::DEFAULT)
            .await
            .expect("Failed to get queue metrics");
        println!(
            "Memory test progress: processed={}, failed={}",
            queue_metrics.processed_tasks, queue_metrics.failed_tasks
        );
    ...
    assert!(
        all_processed,
        "All memory allocation tasks should be processed or failed"
    );
    ...
    let final_metrics = task_queue
        ...
        .expect("Failed to get queue metrics");
    assert!(
        final_metrics.processed_tasks + final_metrics.failed_tasks >= 5,
...
async fn test_redis_pool_exhaustion() {
    ...
    for i in 0..10 {
        ...
        if let Ok(Ok(tq)) = result {
            ...
            println!(
                "Connection {} failed as expected under resource pressure",
                i
            );
        }
    }
...
async fn test_configuration_validation_edge_cases() {
    ...
    assert!(
        ...
        "Should reject max_workers < min_workers"
    );
}
