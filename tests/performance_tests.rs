...
async fn test_high_throughput_task_processing() {
    ...
    loop {
        ...
        if attempts >= 300 {
            // 30 seconds timeout
            panic!(
                "Tasks did not complete in reasonable time. Processed: {}/{}",
    ...
    println!(
        "Processed {} tasks in {:?} ({:.2} tasks/sec)",
        task_count,
        process_time,
        task_count as f64 / process_time.as_secs_f64()
    );
    println!(
        "Total time: {:?} ({:.2} tasks/sec)",
        total_time,
...
async fn test_memory_intensive_workload() {
    ...
    task_queue
        .start_workers_with_registry(2, registry)
        .await
        .expect("Failed to start workers");
    ...
    for i in 0..task_count {
        ...
        task_queue
            .enqueue(task, queue_names::DEFAULT)
            .await
            .expect("Failed to enqueue large task");
    ...
    loop {
        ...
        if attempts >= 200 {
            // 20 seconds timeout
            panic!(
                "Memory intensive tasks did not complete. Processed: {}/{}",
                metrics.processed_tasks, task_count
            );
        }
...
async fn test_cpu_intensive_workload() {
    ...
    registry
        .register_with_name::<CpuIntensiveTask>("cpu_intensive_task")
        .expect("Failed to register task");
    ...
    for i in 0..task_count {
        ...
        task_queue
            .enqueue(task, queue_names::DEFAULT)
            .await
            .expect("Failed to enqueue CPU task");
    }
    ...
    loop {
        ...
        if attempts >= 600 {
            // 60 seconds timeout
            panic!(
                "CPU intensive tasks did not complete. Processed: {}/{}",
                metrics.processed_tasks, task_count
            );
        }
    ...
    let total_time = start_time.elapsed();
    println!(
        "Completed {} CPU-intensive tasks in {:?}",
...
async fn test_concurrent_task_safety() {
    ...
    registry
        .register_with_name::<ConcurrentTask>("concurrent_task")
        .expect("Failed to register task");
    ...
    for i in 0..task_count {
        ...
        task_queue
            .enqueue(task, queue_names::DEFAULT)
            .await
            .expect("Failed to enqueue concurrent task");
    }
    ...
    loop {
        ...
        if attempts >= 200 {
            panic!(
                "Concurrent tasks did not complete. Processed: {}/{}",
                metrics.processed_tasks, task_count
            );
        }
    ...
    assert_eq!(
        final_metrics.processed_tasks, task_count as i64,
...
async fn test_queue_priority_performance() {
    ...
    for i in 0..tasks_per_queue {
        ...
        let task = tokio::spawn(async move {
            ...
            task_queue_clone
                .enqueue(task, queue_names::HIGH_PRIORITY)
                .await
        });
        ...
        let task = tokio::spawn(async move {
            ...
            task_queue_clone
                .enqueue(task, queue_names::LOW_PRIORITY)
                .await
        });
        enqueue_tasks.push(task);
    }
    ...
    for task in enqueue_tasks {
        task.await
            .expect("Enqueue task failed")
            .expect("Failed to enqueue");
    }

    let enqueue_time = start_time.elapsed();
    println!(
        "Enqueued {} tasks to 3 queues in {:?}",
        tasks_per_queue * 3,
        enqueue_time
    );
    ...
    let mut attempts = 0;
    loop {
        let high_metrics = task_queue
            .broker
            .get_queue_metrics(queue_names::HIGH_PRIORITY)
            .await
            .expect("Failed to get high priority metrics");
        let default_metrics = task_queue
            .broker
            .get_queue_metrics(queue_names::DEFAULT)
            .await
            .expect("Failed to get default metrics");
        let low_metrics = task_queue
            .broker
            .get_queue_metrics(queue_names::LOW_PRIORITY)
            .await
            .expect("Failed to get low priority metrics");

        let total_processed = high_metrics.processed_tasks
            + default_metrics.processed_tasks
            + low_metrics.processed_tasks;

        if total_processed >= (tasks_per_queue * 3) as i64 {
            println!(
                "Final metrics - High: {}, Default: {}, Low: {}",
                high_metrics.processed_tasks,
                default_metrics.processed_tasks,
                low_metrics.processed_tasks
            );
            break;
        ...
        if attempts >= 400 {
            // 40 seconds timeout
            panic!(
                "Priority queue tasks did not complete. Total processed: {}/{}",
                total_processed,
                tasks_per_queue * 3
            );
        }
...
async fn test_autoscaler_performance_under_load() {
    ...
    for _ in 0..10 {
        ...
        let mut test_autoscaler =
            AutoScaler::with_config(task_queue.broker.clone(), AutoScalerConfig::default());
        let scaling_action = test_autoscaler
            .decide_scaling_action(&metrics)
            .expect("Failed to decide scaling");

        max_workers_seen = max_workers_seen.max(metrics.active_workers as usize);

        println!(
            "Autoscaler metrics: workers={}, pending={}, queue_pressure={:.2}, action={:?}",
    ...
    println!(
        "Maximum workers seen during load test: {}",
        max_workers_seen
    );
    ...
    loop {
        ...
        if total_handled >= burst_size as i64 {
            println!(
                "All autoscaler test tasks completed: processed={}, failed={}",
        ...
        if attempts >= 300 {
            // Increased timeout for longer-running tasks
    ...
    if max_workers_seen > 1 {
        println!(
            "Autoscaling successfully triggered, max workers: {}",
            max_workers_seen
        );
    } else {
