...
impl AutoScalerConfig {
    pub fn validate(&self) -> Result<(), TaskQueueError> {
        ...
        if triggers.worker_utilization_threshold <= 0.0 || triggers.worker_utilization_threshold > 1.0 {
            return Err(TaskQueueError::Configuration(
...
impl AutoScaler {
    ...
    pub async fn collect_metrics(&self) -> Result<AutoScalerMetrics, TaskQueueError> {
        ...
        let queue_pressure_score = self.calculate_queue_pressure_score(&queue_metrics, active_workers);
    ...
    pub fn decide_scaling_action(&mut self, metrics: &AutoScalerMetrics) -> Result<ScalingAction, TaskQueueError> {
        ...
        let final_action = if self.hysteresis_controller.should_execute_scaling(&proposed_action) {
            #[cfg(feature = "tracing")]
    ...
    fn count_scale_up_signals_static(metrics: &AutoScalerMetrics, thresholds: &ScalingTriggers) -> usize {
        let mut signals = 0;
    ...
    fn count_scale_down_signals_static(metrics: &AutoScalerMetrics, thresholds: &ScalingTriggers) -> usize {
        let mut signals = 0;
    ...
    fn calculate_queue_pressure_score(&self, queue_metrics: &[QueueMetrics], active_workers: i64) -> f64 {
    ...
    fn calculate_worker_utilization(&self, active_workers: i64, queue_metrics: &[QueueMetrics]) -> f64 {
        if active_workers == 0 {
            return 0.0;
        }

        let total_work = queue_metrics.iter()
...
impl AdaptiveThresholdController {
    ...
    pub fn record_performance_snapshot(&mut self, metrics: &AutoScalerMetrics, action: &ScalingAction) {
        ...
        self.performance_history.retain(|s| s.timestamp > cutoff_time);
    ...
    fn adapt_thresholds(&mut self) {
        ...
        let recent_performance = &self.performance_history[self.performance_history.len().saturating_sub(10)..];
        
        let avg_success_rate = recent_performance.iter()
            .map(|s| s.success_rate)
            .sum::<f64>() / recent_performance.len() as f64;

        let avg_utilization = recent_performance.iter()
        ...
        self.current_thresholds.queue_pressure_threshold = self.current_thresholds.queue_pressure_threshold.clamp(0.1, 2.0);
        self.current_thresholds.worker_utilization_threshold = self.current_thresholds.worker_utilization_threshold.clamp(0.1, 0.95);
    }

    pub fn get_adaptation_status(&self) -> String {
        format!("Thresholds adapted {} times, {} performance samples",
                0, self.performance_history.len())
    }
}
...
mod tests {
    ...
    fn test_autoscaler_config_validation_invalid_scale_down_count() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Scale down count must be between 1 and 50"));
    }
    ...
    fn test_autoscaler_config_validation_invalid_learning_rate() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Learning rate must be between 0.01 and 1.0"));
    }
    ...
    fn test_autoscaler_config_serialization() {
        ...
        assert_eq!(config.scaling_triggers.queue_pressure_threshold, deserialized.scaling_triggers.queue_pressure_threshold);
        assert_eq!(config.scaling_triggers.worker_utilization_threshold, deserialized.scaling_triggers.worker_utilization_threshold);
        assert_eq!(config.scaling_triggers.task_complexity_threshold, deserialized.scaling_triggers.task_complexity_threshold);
        assert_eq!(config.scaling_triggers.error_rate_threshold, deserialized.scaling_triggers.error_rate_threshold);
        assert_eq!(config.scaling_triggers.memory_pressure_threshold, deserialized.scaling_triggers.memory_pressure_threshold);
        assert_eq!(config.enable_adaptive_thresholds, deserialized.enable_adaptive_thresholds);
        assert_eq!(config.learning_rate, deserialized.learning_rate);
        assert_eq!(config.adaptation_window_minutes, deserialized.adaptation_window_minutes);
        assert_eq!(config.scale_up_cooldown_seconds, deserialized.scale_up_cooldown_seconds);
        assert_eq!(config.scale_down_cooldown_seconds, deserialized.scale_down_cooldown_seconds);
        assert_eq!(config.consecutive_signals_required, deserialized.consecutive_signals_required);
        assert_eq!(config.target_sla.max_p95_latency_ms, deserialized.target_sla.max_p95_latency_ms);
        assert_eq!(config.target_sla.min_success_rate, deserialized.target_sla.min_success_rate);
        assert_eq!(config.target_sla.max_queue_wait_time_ms, deserialized.target_sla.max_queue_wait_time_ms);
        assert_eq!(config.target_sla.target_worker_utilization, deserialized.target_sla.target_worker_utilization);
    }
    ...
    fn create_test_autoscaler_metrics_with_high_pressure(active_workers: i64, total_pending_tasks: i64) -> AutoScalerMetrics {
        let queue_pressure_score = if active_workers > 0 {
            total_pending_tasks as f64 / active_workers as f64
        } else {
            total_pending_tasks as f64
    ...
    fn create_test_autoscaler_metrics_with_low_pressure(active_workers: i64, total_pending_tasks: i64) -> AutoScalerMetrics {
        let queue_pressure_score = if active_workers > 0 {
            total_pending_tasks as f64 / active_workers as f64
        } else {
            total_pending_tasks as f64
    ...
    fn test_scaling_action_scale_up() {
        ...
        let action1 = autoscaler.decide_scaling_action(&metrics).expect("Failed to decide scaling action");
        let action2 = autoscaler.decide_scaling_action(&metrics).expect("Failed to decide scaling action");
        
        // The second action should trigger scaling due to consecutive signals
        match action2 {
            ScalingAction::ScaleUp(count) => assert_eq!(count, 2),
            _ => panic!("Expected ScaleUp action, got {:?}. First action was {:?}", action2, action1),
        }
    }
    ...
    fn test_scaling_action_scale_down() {
        ...
        match action2 {
            ...
            _ => panic!("Expected ScaleDown action, got {:?}. First action was {:?}", action2, action1),
        }
    }
    ...
    fn test_scaling_action_limited_by_max_workers() {
        ...
        match action2 {
            ...
            _ => panic!("Expected ScaleUp action limited by max workers, got {:?}. First action was {:?}", action2, action1),
        }
    }
    ...
    fn test_scaling_action_limited_by_min_workers() {
        ...
        match action2 {
            ...
            _ => panic!("Expected ScaleDown action limited by min workers, got {:?}. First action was {:?}", action2, action1),
        }
    }
    ...
    fn test_scaling_action_zero_workers() {
        ...
        match action2 {
            ScalingAction::ScaleUp(count) => assert_eq!(count, 2),
            _ => panic!("Expected ScaleUp action with zero workers, got {:?}. First action was {:?}", action2, action1),
        }
    }
    ...
    fn test_autoscaler_config_clone() {
        ...
        assert_eq!(original.scaling_triggers.queue_pressure_threshold, cloned.scaling_triggers.queue_pressure_threshold);
        assert_eq!(original.scaling_triggers.worker_utilization_threshold, cloned.scaling_triggers.worker_utilization_threshold);
        assert_eq!(original.scaling_triggers.task_complexity_threshold, cloned.scaling_triggers.task_complexity_threshold);
        assert_eq!(original.scaling_triggers.error_rate_threshold, cloned.scaling_triggers.error_rate_threshold);
        assert_eq!(original.scaling_triggers.memory_pressure_threshold, cloned.scaling_triggers.memory_pressure_threshold);
        assert_eq!(original.enable_adaptive_thresholds, cloned.enable_adaptive_thresholds);
        assert_eq!(original.learning_rate, cloned.learning_rate);
        assert_eq!(original.adaptation_window_minutes, cloned.adaptation_window_minutes);
        assert_eq!(original.scale_up_cooldown_seconds, cloned.scale_up_cooldown_seconds);
        assert_eq!(original.scale_down_cooldown_seconds, cloned.scale_down_cooldown_seconds);
        assert_eq!(original.consecutive_signals_required, cloned.consecutive_signals_required);
        assert_eq!(original.target_sla.max_p95_latency_ms, cloned.target_sla.max_p95_latency_ms);
        assert_eq!(original.target_sla.min_success_rate, cloned.target_sla.min_success_rate);
        assert_eq!(original.target_sla.max_queue_wait_time_ms, cloned.target_sla.max_queue_wait_time_ms);
        assert_eq!(original.target_sla.target_worker_utilization, cloned.target_sla.target_worker_utilization);
    }
}
